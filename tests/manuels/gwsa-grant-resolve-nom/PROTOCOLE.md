# Test manuel — `gwsa grant` résout un dossier par son NOM

Ce test prouve une seule chose, mais à fond : **`gwsa grant <alias> <dossier>`
...
3. **Pas de Touch ID gaspillé** — la résolution a lieu **avant** l'élicitation
   signée (bin/gwsa `cmd_grant`) : un nom invalide ne doit **jamais** faire
   apparaître de dialogue Touch ID.

Référence code : `resolve_folder()` dans `bin/gwsa` (~ligne 84) et son appel
...
*Testing*), prévoir `gwsa add <alias>` en cours de route (erreur `exit code 2`).

## Déroulé (ce que l'agent doit faire)

Conventions : commandes agent via `GWSA_CLIENT=claude-code gwsa …` (jamais
...
- `profiles_list` (MCP) ou `gwsa list` : confirmer que `perso` et `mw` existent.
- `gwsa strongauth status` : annoncer si Touch ID sera demandé — cette réponse
...
- Tenter `gwsa perso auth status` → refus « verrouillé 🔒 » **attendu**.
...
```bash
gwsa grant perso ZZ-TESTS 1
gwsa grant mw ZZ-TESTS 1
```
...
- [ ] `gwsa grants <alias>` liste bien la zone, avec son échéance.
...
```bash
gwsa grant perso <ID_ZZ-TESTS_perso> 1
```
...
- [ ] `gwsa grants perso` ne contient **pas** de doublon : re-granter la même
...
```bash
gwsa grant perso ZZ-INEXISTANT 1     # 1. nom introuvable
gwsa grant perso ZZ-AMBIGU 1         # 2. deux dossiers homonymes
gwsa grant mw ZZ-AMBIGU 1            # 3. existe chez perso, pas chez mw
```
...
- [ ] `gwsa grants perso` / `gwsa grants mw` sont **inchangés** après ces
...
```bash
gwsa grant perso ZZ-TESTS 0          # durée invalide
gwsa grant perso ZZ-TESTS 999        # durée hors bornes (max 168)
```
...
2. `gwsa grant mw ZZ-TESTS 1` → doit échouer (« introuvable ou ambigu »).
3. **L'humain** le restaure depuis la corbeille.
4. `gwsa grant mw ZZ-TESTS 1` → repasse.
...
```bash
gwsa grant perso revoke <ID_ZZ-TESTS_perso>
gwsa grant mw revoke <ID_ZZ-TESTS_mw>
```
...
| `gwsa grant` accepte `ZZ-AMBIGU` | **Régression** — l'ambiguïté doit être refusée | Vérifier qu'il y a bien 2 dossiers homonymes, sinon fiche |
