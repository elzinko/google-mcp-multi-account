...
description: "Piloter plusieurs comptes Google (Gmail, Drive, Calendar, Docs, Sheets, Tasks) via le wrapper mag. Utiliser dès qu'une tâche vise un compte Google précis ou plusieurs comptes : lire/envoyer des mails, gérer des fichiers Drive, consulter des agendas."
---

# Multi-comptes Google avec mag / MCP
...
Le wrapper `mag` reste pour l'admin et les skills shell.

## Workflow

1. Tool MCP `profiles_list` — ou `mag list` — voir les profils.
2. Choisir l'alias ; si verrouillé → `access_request` kind=unlock (humain).
3. Données : tools `gmail_*` / `drive_*`. Shell de secours :
   `mag <alias> <service> …` (jamais `gws` nu).
...
2. Attendre que l'utilisateur exécute `mag grant …` (ou admin).
...
2. **Partage** : `drive_permissions_create` sur A (`email` = compte B,
   ...
   (l'humain active via admin ou `mag policy`).
...
Binaire ou export d'un Doc existant : shell `mag <alias> drive files export …`
...
- Exit code 2 = token expiré → `mag add <alias>`.
