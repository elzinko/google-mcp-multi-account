...
mal, et ce qui pourrait le rendre inutile. Il complète
[`threat-model.md`](threat-model.md) (centré sécurité) par une vue plus large :
produit, ingénierie, stratégie.

*Écrit le 2026-07-24. Chaque limite renvoie à la fiche du [backlog](../features/)
quand un travail est prévu.*
...
- **Il est petit et sans dépendance lourde.** ~4 000 lignes, stdlib pure (Python,
...
Le projet le documente lui-même dans [`threat-model.md`](threat-model.md). La vraie
fermeture consiste à sortir les credentials du périmètre de l'agent : c'est la
fiche [0003](../features/0003-vault-credentials-hors-perimetre-agent.md), **pas
encore faite**.

À l'inverse, pour un client **sans shell** (Claude Desktop en MCP stdio), la
barrière tient bien mieux.
...
Trousseau (clé de chiffrement), Touch ID, chemins Homebrew, `stat -f`, `open` : le
chemin de production ne fonctionne que sur macOS Apple Silicon. Le README le dit
désormais. Le rendre portable est la fiche
[0015](../features/0015-cross-platform-hors-macos.md), sous l'épic
[0014](../features/0014-generaliser-autres-utilisateurs.md).
...
expose une centaine d'outils sur 12 services. C'est la fiche
[0018](../features/0018-couverture-mcp-elargie.md).
...
Documentation en français, installation par `git clone` + symlink codé en dur, pas
de release, pas de paquet. Un format standard existe pourtant pour distribuer un
serveur MCP en un clic : les [Desktop Extensions
`.mcpb`](https://www.anthropic.com/engineering/desktop-extensions). Ouvrir le projet
à d'autres (doc anglaise [0016](../features/0016-documentation-anglaise.md),
packaging [0017](../features/0017-packaging-installable.md)) est regroupé sous
l'épic [0014](../features/0014-generaliser-autres-utilisateurs.md).
...
- [`../features/`](../features/) — le backlog : chaque limite ci-dessus y a sa fiche.
