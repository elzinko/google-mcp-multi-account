...
- **.email** — un fichier par profil qui mémorise l'email, lisible **même profil verrouillé** (pour le diagnostic). Les lectures gateway/MCP ne relancent jamais `gws` ; seul un backfill humain (`gma list`, admin) peut le recalculer pour un ancien profil (ADR-0002).
- **gateway** — `gateway/` : la couche qui vérifie **verrou + policy** avant chaque appel à Google.
- **gma** — la CLI **de ce projet** — le poste de pilotage humain (`add`, `lock`/`unlock`, `grant`, `policy`, `wire`). Anciennement `gwsa` (alias déprécié conservé).
...
- **jeton de session** — le secret créé par `gma session open` (geste **signé**) qui identifie une **session** ; l'assistant le présente à chaque appel. Sans jeton : aucun accès aux données, seulement la demande de création (ADR-0007).
...
- **strongauth / Touch ID** — mode **optionnel** (`gma strongauth on`) : une fois activé, chaque unlock/grant/connexion exige une empreinte **Touch ID** qui signe l'action précise.
