...
**Date** : 2026-07-28  
**Statut** : accepté (implémentation locale mag ; second consommateur whatsapp-group-mcp#0007 non requis pour le chemin macOS)

> **TL;DR** — **Quand le mode strongauth est activé** (`mag strongauth on`), chaque action sensible (débloquer un accès, ouvrir un dossier partagé, approuver un projet) doit être validée par une empreinte Touch ID qui **signe cryptographiquement** la description de cette action précise — au lieu de l'ancien contrôle qui vérifiait seulement qu'un humain était présent — pour qu'une approbation ne puisse ni être rejouée ni servir à autoriser une autre action.

## Contexte

`mag strongauth` v1 = presence check (`touchid.swift`). La fiche 0001 exige un
...
1. **Payload canonique unique** (`gateway/elicitation.py`) : `action`, `alias`,
   ...
   (SHA-256 + ECDSA P-256).
2. **Clé** : enrôlement `mag elicitation enroll` tente dans l'ordre Secure Enclave
...
4. **Tests CI / Linux** : `GWSA_ELICITATION_MOCK=1` + `mag elicitation enroll --mock`
...
- `strongauth on` sans enrôlement → refus avec message `mag elicitation enroll`.
...
- Vérification au moment de la commande humaine `mag` ; le broker fait confiance
...
- `scripts/elicitation-sign.swift`, `gateway/elicitation.py`, `bin/mag`
