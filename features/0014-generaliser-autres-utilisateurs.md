...
- **macOS Apple Silicon câblé en dur** — Trousseau, Touch ID, `/opt/homebrew`,
  `stat -f`, `open` : hors macOS, non fonctionnel **et non annoncé** (→ 0015).
- **Documentation 100 % française** — rédhibitoire pour une audience GitHub (→ 0016).
- **Installation par `git clone` + symlink codé en dur + édition manuelle de
  config** — pas de release, pas de packaging (→ 0017).
- **Couverture MCP étroite** — 9 tools dont 3 méta ; Calendar/Docs/Sheets/Tasks
  ...
  fonctionnalité la plus pauvre du marché (→ 0018).
...
travail propre : il coordonne 0015 (cross-platform), 0016 (doc EN), 0017
(packaging), 0018 (couverture MCP). Jamais tirable — on tire ses enfants.
...
- [ ] Enfants livrés : 0015, 0016, 0017 (0018 = confort, hors chemin critique).
