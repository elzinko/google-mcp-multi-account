---
id: 0011
title: "`mag admin` — démarrer/arrêter l'interface web en un geste, proposé par l'élicitation"
type: feature
priority: P2
version:
epic:
...
1. Sous-commande **`mag admin [stop]`** : démarre `admin/server.js` détaché
...
2. Les **messages d'élicitation** (locked, unlock, grant) citent « démarrer :
   `mag admin` » à côté de l'URL — tout client LLM sait la proposer ; dans
...
- [x] `mag admin` démarre (détaché, idempotent) et ouvre le navigateur ;
      `mag admin stop` arrête proprement ; relance sûre.
