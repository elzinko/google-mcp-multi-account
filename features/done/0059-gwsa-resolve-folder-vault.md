---
id: 0059
title: mag ne résout plus un dossier par son nom depuis le vault (grant / policy zone)
type: bug
priority: P1
version:
epic:
...
`4304a1e`, 28/07), **`mag grant <alias> <NOM_DE_DOSSIER>` échoue toujours** :

```
$ GWSA_CLIENT=claude-code mag grant perso ZZ-TESTS 1
mag : dossier « ZZ-TESTS » introuvable ou ambigu — donne son ID (fin de l'URL du dossier dans Drive)
```
...
**Cause.** `resolve_folder()` (`bin/mag:84-101`) appelle `gws` avec
...
`gateway/vault.py:112`) et fonctionne. `bin/mag` est resté sur `profile_dir`.
...
- `cmd_grant` (`bin/mag:432`) — accorder une zone Drive par nom
- `cmd_policy … zone` (`bin/mag:347`) — déclarer une zone par nom

Et le même pattern (`CONFIG_DIR=profile_dir` + `gws`) subsiste ailleurs dans
`bin/mag` : ligne 169 (`profile_email`, déjà rustiné en cache-only par
...
`tests/manuels/mag-grant-resolve-nom/` (Phase 2), sur `perso` et `mw`.
...
la garde… mais seulement pour `gateway/`. `bin/mag` n'a jamais été mis en
...
1. **Faire passer `resolve_folder` par le broker**, conformément à la fiche
   ...
   `$SYS_PYTHON`, utilisé par **tous** les appels `gws` de `bin/mag`.
2. **Ne plus avaler l'erreur.** `2>/dev/null` transforme une panne d'auth en
   ...
   (« profil non authentifié — `mag add <alias>` »), ne pas accuser le dossier.
3. **Test de non-régression** dans `scripts/test.sh` : l'invariant existant
   ...
   `scripts/test.sh:1049`) ne couvre pas `bin/mag`. L'étendre.

## Critères d'acceptation

- [ ] `mag grant <alias> ZZ-TESTS 1` résout le nom et affiche `« ZZ-TESTS » (<ID>)`.
- [ ] Le même nom résout vers un **ID différent** sur deux comptes (pas de fuite inter-profils).
- [ ] `mag policy <alias> zone <NOM>` résout aussi.
...
- [ ] Les autres appels `gws` de `bin/mag` sont audités et passent par le même helper.
- [ ] `scripts/test.sh` échoue si un appel `gws` de `bin/mag` repart sur `profile_dir`.
- [ ] Le test manuel `mag-grant-resolve-nom` passe de bout en bout.
...
- Le test manuel qui a attrapé le bug est versionné dans
  `tests/manuels/mag-grant-resolve-nom/`.
