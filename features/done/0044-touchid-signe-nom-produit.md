...
Constaté pendant le test manuel `mag-grant-resolve-nom` (2026-07-29) : **sous
...
`bin/mag` mais **jamais lu** côté Python (code mort depuis le passage à la
...
réutilisée par `bin/mag` (compilation) et `scripts/test.sh` (assertion) ; artefact
...
- Découvert via le test manuel `mag-grant-resolve-nom` (comme 0032 est né du test drive-2-comptes).
