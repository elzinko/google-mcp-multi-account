...
def setup_status() -> dict[str, Any]:
    ...
    if any(a["connected"] and not a["email"] for a in accounts):
        next_actions.append("mag list   # renseigner l'email des profils existants (métadonnée .email, une fois)")
    for a in accounts:
        if a["locked"]:
            next_actions.append(f"mag unlock {a['alias']} 30   # profil verrouillé (accès sur demande)")
    if not accounts:
        next_actions.append("mag add <alias> <email>   # connecter un premier compte")
